...
윈도우가 이동하면서 새로 들어오는 값에 따라 최대값이 결정되겠지만, 불행히도 새로 
들어오는 원소에 상관없이 절대 최대값이 될 수 없는 원소들이 있다.
...
첫 번째 경우는 자명하다. 자신이 윈도우에서 빠져나가기 전까지 자신보다 큰 값이
윈도우에 계속 존재하므로 최대값이 될 수 없다. 두 번째 경우는 현재 시점의 최대값이
자신보다 왼쪽에 있기 때문에, 최대값이 빠져나간 후에도 윈도우 안에 남아있겠지만,
자신의 오른쪽에 더 큰 값이 있기 때문에, 최대값이 빠져나가는 순간 첫 번째 경우가 된다.
